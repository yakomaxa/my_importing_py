...
PYMOL API
    ...
    fetchHosts = {
        "pdb" : "https://files.pdbj.org/pub/pdb/data/structures/divided/",
    }
    ...
    def _fetch(code, name,
               state, finish, discrete, multiplex, zoom,
               type, path,
    ...
    def _multifetch(code,name,state,finish,discrete,multiplex,zoom,type,path,file,quiet,_self):
        ...
        for obj_code in code_list:
            ...
            chain = None
            if (len(obj_code) > 4 and type in ('cif', 'pdb', 'mmtf') and
...
DESCRIPTION
    ...
    # pymol_mtz_loader.py
    # Adds:
    #   - load_mtz path/to/file.mtz [, name]
    #   - fetch_mtz 9QFY [, kind=2fo-fc] [, state=1] [, name]
    #
    # Dependencies: gemmi CLI (https://gemmi.readthedocs.io/) available as `gemmi` on PATH.
    ...
    def _ensure_gemmi():
        if shutil.which("gemmi") is None:
            raise CmdException(
                "Cannot find 'gemmi' executable. Please install gemmi and ensure it is on your PATH.\n"
                "For example: pip install gemmi  (adds CLI on many platforms), or use conda-forge."
            )


    def _run_sf2map(mtz_path: str, out_ccp4: str, num: int = 3):
        _ensure_gemmi()
        try:
            # gemmi auto-detects common columns (FWT/PHWT etc.) for 2Fo–Fc by default
            # If needed later, you can add flags here (e.g., --col F=FWT,PH=PHWT).
            subprocess.run(
                ["gemmi", "sf2map", "-s " + str(num), mtz_path, out_ccp4],
                check=True,
    ...
    def _download_to_temp(url: str, suffix: str = "") -> str:
        ...
        if curl is None:
            raise CmdException("curl not found on PATH (required for robust downloads)")

        try:
            # -L follow redirects, -s silent, -S show errors, -f fail on HTTP errors
            subprocess.run(
                [curl, "-L", "-sS", "-f", url, "-o", tmp_path],
                check=True
            )
        except subprocess.CalledProcessError as e:
            try:
                ...
                pass
            raise CmdException(f"Failed to download {url} via curl (exit {e.returncode})")
    ...
    def load_mtz_gemmi(filename: str, name: str = "", num: int = 6, quiet: int = 0):
        """
        Convert an MTZ (structure-factor coefficients) to CCP4 map via gemmi and load it.

        Usage:
            load_mtz path/to/file.mtz [, name]

        Notes:
          - Uses gemmi's default column detection (works for common 2Fo–Fc/Fo–Fc coefficient MTZs).
          - Creates a temporary CCP4 file and removes it after loading.
        """
        mtz_path = cmd.exp_path(filename)

        if not name:
            name = _safe_name_from_path(filename)

        # Build a temp CCP4 path
        fd, out_ccp4 = tempfile.mkstemp(suffix=".ccp4")
        os.close(fd)

        try:
            _run_sf2map(filename, out_ccp4, num)
            # Load as a map; PyMOL picks up CCP4 format automatically
            cmd.load(out_ccp4, name)
            if not int(quiet):
                print(f"[load_mtz] Loaded CCP4 map '{name}' from: {filename}")
        finally:
            # Clean up the temp CCP4 file
            try:
                os.remove(out_ccp4)
            except Exception:
                pass
    ...
    import tempfile
    import gemmi    
    def load_mmjson(filename, object_name=None, _self=cmd):
        # Use input filename (without extension) as base
        base_name = os.path.splitext(os.path.basename(filename))[0]
        print("loading coordinate from mmJSON")
        with tempfile.TemporaryDirectory() as tmpdir:
            cif_path = os.path.join(tmpdir, f"{base_name}.cif")
            # Read mmJSON and convert to CIF
            structure = gemmi.read_structure(filename)
            structure.make_mmcif_document().write_file(cif_path)
            # Load into PyMOL
            _self.load(cif_path, object_name if object_name else base_name)
            # Register as PyMOL command
    #cmd.extend("load_mmjson", load_mmjson)
